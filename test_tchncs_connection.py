...
import certifi
import aiohttp
from nio import AsyncClient, AsyncClientConfig, LoginError, LoginResponse
...
async def test_server(homeserver, username):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"{homeserver}/_matrix/client/versions", timeout=aiohttp.ClientTimeout(total=10)) as resp:
    ...
    try:
        ...
        try:
            versions_response = await asyncio.wait_for(
                client.versions(),
                timeout=10.0
            )
        ...
        login_response = await asyncio.wait_for(
            client.login(password=password, device_name="test-device"),
            timeout=30.0
        )
    ...
    try:
        ...
        login_response = await asyncio.wait_for(
            client2.login(password=password, device_name="test-device-2"),
            timeout=30.0
        )
