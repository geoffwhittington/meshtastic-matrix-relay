...
class TestDBUtilsEdgeCases(unittest.TestCase):
    ...
    def test_save_shortname_constraint_violation(self):
        ...
        with patch('sqlite3.connect') as mock_connect:
            mock_conn = MagicMock()
            mock_conn.cursor.return_value.execute.side_effect = sqlite3.IntegrityError("constraint violation")
            mock_connect.return_value.__enter__.return_value = mock_conn
    ...
    def test_get_message_map_by_matrix_event_id_unicode_error(self):
        ...
        with patch('sqlite3.connect') as mock_connect:
            ...
            mock_cursor.execute.side_effect = UnicodeDecodeError("utf-8", b"", 0, 1, "invalid")
            mock_connect.return_value.__enter__.return_value = mock_conn
    ...
    def test_wipe_message_map_transaction_rollback(self):
        ...
        with patch('sqlite3.connect') as mock_connect:
            mock_conn = MagicMock()
            mock_conn.cursor.return_value.execute.side_effect = [None, sqlite3.Error("Transaction failed")]
            mock_connect.return_value.__enter__.return_value = mock_conn
    ...
    def test_get_plugin_data_for_node_memory_error(self):
        ...
        with patch('sqlite3.connect') as mock_connect:
            mock_conn = MagicMock()
            mock_conn.cursor.return_value.fetchall.side_effect = MemoryError("Out of memory")
            mock_connect.return_value.__enter__.return_value = mock_conn
    ...
    def test_delete_plugin_data_foreign_key_constraint(self):
        ...
        with patch('sqlite3.connect') as mock_connect:
            mock_conn = MagicMock()
            mock_conn.cursor.return_value.execute.side_effect = sqlite3.IntegrityError("foreign key constraint")
            mock_connect.return_value.__enter__.return_value = mock_conn
    ...
    def test_database_path_caching_race_condition(self):
        ...
        with patch('mmrelay.db_utils.get_data_dir', side_effect=side_effect_clear_cache):
            path1 = get_db_path()
            path2 = get_db_path()
    ...
    def test_database_initialization_partial_failure(self):
        ...
        with patch('sqlite3.connect') as mock_connect:
            ...
            mock_cursor.execute.side_effect = [None, sqlite3.Error("Table creation failed")]
            mock_connect.return_value.__enter__.return_value = mock_conn
