...
class TestErrorBoundaries(unittest.TestCase):
    ...
    def test_plugin_failure_isolation(self):
        ...
        failing_plugin.handle_meshtastic_message = AsyncMock(side_effect=Exception("Plugin crashed"))
        ...
        with patch('mmrelay.plugin_loader.load_plugins', return_value=[failing_plugin, working_plugin]):
    ...
    def test_database_failure_graceful_degradation(self):
        ...
        with patch('mmrelay.db_utils.get_longname', side_effect=Exception("Database error")):
            with patch('mmrelay.db_utils.get_shortname', side_effect=Exception("Database error")):
                with patch('mmrelay.plugin_loader.load_plugins', return_value=[]):
                    with patch('mmrelay.matrix_utils.matrix_relay') as mock_matrix_relay:
                        with patch('mmrelay.meshtastic_utils.logger') as mock_logger:
                            ...
                            self.assertIn('Test Node', call_args[1])  # Should use interface longName
    ...
    def test_matrix_relay_failure_recovery(self):
        ...
        with patch('mmrelay.plugin_loader.load_plugins', return_value=[]):
            with patch('mmrelay.matrix_utils.matrix_relay', side_effect=matrix_relay_side_effect):
                with patch('mmrelay.meshtastic_utils.logger') as mock_logger:
                    # Set up config
                    import mmrelay.meshtastic_utils
                    mmrelay.meshtastic_utils.config = {
    ...
    def test_cascading_failure_prevention(self):
        ...
        with patch('mmrelay.db_utils.get_longname', side_effect=Exception("DB failure")):
            with patch('mmrelay.db_utils.get_shortname', side_effect=Exception("DB failure")):
                with patch('mmrelay.plugin_loader.load_plugins') as mock_load_plugins:
                    ...
                    with patch('mmrelay.matrix_utils.matrix_relay', side_effect=Exception("Matrix failure")):
                        with patch('mmrelay.meshtastic_utils.logger') as mock_logger:
                            # Set up config
                            import mmrelay.meshtastic_utils
                            mmrelay.meshtastic_utils.config = {
    ...
    def test_transient_failure_recovery(self):
        ...
        try:
            ...
            for i in range(5):
                queue.enqueue(transient_failure_function, description=f"Transient test {i}")
    ...
    def test_error_propagation_limits(self):
        ...
        async def async_test():
            ...
            mock_plugin.handle_room_message = AsyncMock(side_effect=Exception("Plugin error"))
    ...
    def test_resource_exhaustion_handling(self):
        ...
        def memory_intensive_function():
            ...
            try:
                ...
                large_data = [0] * (10**6)  # 1 million integers
