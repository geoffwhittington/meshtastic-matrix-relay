...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_complete_matrix_to_meshtastic_flow(self):
        ...
        async def async_test():
            ...
            with patch('mmrelay.plugin_loader.load_plugins') as mock_load_plugins:
                ...
                mock_plugin = MagicMock()
                mock_plugin.handle_room_message = AsyncMock(return_value=True)  # Intercepts
                mock_load_plugins.return_value = [mock_plugin]
    ...
    def test_plugin_chain_processing(self):
        ...
        mock_plugin3.handle_meshtastic_message = AsyncMock(return_value=True)  # Intercepts
...
"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('mmrelay.db_utils.get_db_path', return_value=db_path):
                ...
                from mmrelay.db_utils import (
                    save_longname, get_longname,
                    save_shortname, get_shortname,
                    store_message_map, get_message_map_by_meshtastic_id
                )
                ...
                store_message_map('mesh123', 'matrix456', '!room:matrix.org', 'test message')
    ...
    def test_multi_room_message_routing(self):
        ...
        config = {
            'matrix_rooms': [
                ...
                {'id': '!room3:matrix.org', 'meshtastic_channel': 1}  # Different channel
            ]
        }
        
        packet = {
            'decoded': {
                'text': 'Multi-room test',
                'portnum': 1
            },
            'fromId': '!12345678',
    ...
    def test_service_lifecycle_simulation(self):
        ...
        with patch('mmrelay.setup_utils.get_executable_path', return_value='/usr/bin/mmrelay'):
            with patch('mmrelay.setup_utils.create_service_file', return_value=True):
                with patch('mmrelay.setup_utils.reload_systemd_daemon', return_value=True):
