...
class TestMain(unittest.TestCase):
    ...
    def test_main_with_message_map_wipe(self):
        """
        Verify that the message map wipe function is called when the configuration enables wiping on restart.
        
        This test checks both new and legacy configuration formats to ensure the wipe logic is triggered appropriately.
        """
    ...
    def test_run_main(
        ...
        """
        Tests that run_main successfully loads configuration, sets logging, prints the banner, configures debug logging, runs the main function, and returns 0 on successful execution.
        """
        # Mock arguments
        mock_args = MagicMock()
    ...
    def test_run_main_keyboard_interrupt(self, mock_asyncio_run, mock_load_config):
        """
        Test that run_main returns 0 when a KeyboardInterrupt occurs during execution, confirming graceful shutdown behavior.
        """
        # Mock config loading
        mock_load_config.return_value = self.mock_config
...
class TestRunMain(unittest.TestCase):
    ...
    def test_run_main_success(
        ...
        Ensures that the banner is printed, configuration is loaded, and the main async function is executed when provided with correct arguments.
        """
        # Mock configuration
        mock_config = {
        ...
        def mock_run(coro):
            ...
            """
            Closes the given coroutine if possible to prevent warnings, then returns None.
            
            Parameters:
                coro: A coroutine object to be closed if it supports the 'close' method.
    ...
    def test_run_main_with_log_level(
        ...
        Verifies that specifying a log level in the arguments overrides the logging level in the configuration and that run_main returns 0 on success.
        """
        mock_config = {
            "matrix": {"homeserver": "https://matrix.org"},
            "meshtastic": {"connection_type": "serial"},
        ...
        def mock_run(coro):
            """
            Closes the provided coroutine to suppress "never awaited" warnings during testing.
...
class TestMainFunctionEdgeCases(unittest.TestCase):
    ...
    def test_main_with_database_wipe_new_format(self):
        """
        Test that the database wipe function is called when the new configuration format enables `wipe_on_restart`.
        
        Verifies that the logic correctly detects the `wipe_on_restart` flag in the `database.msg_map` section and triggers the wipe operation.
        """
    ...
    def test_main_with_database_wipe_legacy_format(self):
        """
        Test that the database wipe logic is triggered when using the legacy configuration format.
        
        Verifies that setting `wipe_on_restart` in the legacy `db.msg_map` config causes the database wipe function to be called.
        """
    ...
    def test_main_with_custom_message_delay(self):
        """
        Test that a custom message delay value from the configuration is correctly extracted and passed to the message queue startup function.
        """
    ...
    def test_main_no_meshtastic_client_warning(self):
        """
        Test that update functions are not called when the Meshtastic client is None.
        
        Verifies that the main logic correctly skips updating longnames and shortnames if the Meshtastic client is not initialized.
        """
