...
def is_running_as_service():
    """
    Returns True if the application is running as a systemd service.
    
    Checks for the presence of the INVOCATION_ID environment variable or verifies if the parent process is systemd to determine service execution context.
    """
...
def connect_meshtastic(passed_config=None, force_connect=False):
    ...
    Parameters:
        passed_config (dict, optional): Configuration dictionary for the Meshtastic connection.
...
async def reconnect():
    """
    Asynchronously attempts to reconnect to the Meshtastic device using exponential backoff until successful or shutdown is initiated.
    
    Reconnection starts with a 10-second delay, doubling after each failure up to a maximum of 5 minutes. If not running as a service, a progress bar is shown during the wait. The process stops immediately if shutdown is triggered or reconnection succeeds.
    """
...
async def check_connection():
    ...
    For non-BLE connections, attempts to retrieve device metadata at configured intervals to verify connectivity. If the check fails or the firmware version is missing, initiates reconnection unless already in progress. BLE connections are excluded from periodic checks due to real-time disconnection detection. The function continues running until shutdown is requested, with behavior controlled by configuration settings.
    """
